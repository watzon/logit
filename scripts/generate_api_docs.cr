...
# Generate API documentation from Crystal doc JSON output
#
# Usage:
#   crystal run scripts/generate_api_docs.cr
#
# This script:
# 1. Runs `crystal doc -f json` to generate documentation JSON
# 2. Parses the JSON and generates markdown files in docs/api/
# 3. Creates an index with all types organized by category

require "json"
require "file_utils"

# Type representation from Crystal doc JSON
struct DocType
...
class APIDocGenerator
  ...
  EXCLUDED_TYPES = ["Macros", "PatternMatcher", "Utils", "BufferedIO", "Instrumentation"]

  # Order for type categories
  TYPE_ORDER = {
    "module"     => 0,
    "class"      => 1,
    "struct"     => 2,
  ...
  def generate
    ...
    # Find the Logit module
    logit_module = @root.program.types.try(&.find { |t| t.name == "Logit" })
    return puts "Error: Logit module not found" unless logit_module

    types = collect_types(logit_module)
  ...
  private def collect_types(mod : DocType, prefix = "") : Array(DocType)
    ...
    mod.types.try(&.each do |type|
      ...
      # Collect nested types (like Backend::Console)
  ...
  private def render_type(type : DocType) : String
    String.build do |io|
      ...
      if loc = type.locations.try(&.first?)
        ...
        io << "(https://github.com/watzon/logit/blob/main/" << loc.filename << "#L" << loc.line_number << ")*\n\n"
      end
      ...
      if instance_methods = type.instance_methods
        # Filter out property accessors and undocumented methods
  ...
  private def render_method(io : IO, method : DocType::Method, separator : String)
    ...
    if loc = method.location
      io << "*[View source](https://github.com/watzon/logit/blob/main/"
  ...
  private def generate_index(types : Array(DocType))
    content = String.build do |io|
      io << "# API Reference\n\n"
      io << "Complete API documentation for Logit, auto-generated from source code.\n\n"
